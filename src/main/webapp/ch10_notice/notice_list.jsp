...
<head>
  ...
  <%
    ...
    Statement stmt = conn.createStatement();	// DB에 쿼리문를 날려주는 statement 객체 생성
		...
		String query1 = "select count(*) from notice;";
    ...
    String query2 = "select * from notice order by id desc limit " + ((Integer.parseInt(no_page)-1)*size_page) + ", " + size_page + ";";
...
<body>
  <div class="container">
        ...
        <%
          while(rset2.next()) {
            out.println("<tr>");											
              ...
              out.println("<td><a href='notice_view.jsp?id=" + rset2.getString("id") + "'>" + rset2.getString("title") + "</a></td>");
    ...
    <div class="div-button">
      <input type="button" value="신규" onclick="window.location='notice_insertForm.jsp'">
    </div>
    ...
    <div class="pagination">
      <%
        ...
        if ( no_page_start != 1) {					// 첫 페이징 그룹이 아닐 때만 <와 << 표시
          out.println("<tr><td><a href='notice_list.jsp'>&lt;&lt;</a></td>");			// << 출력하고 첫 페이지로 이동 링크
          out.println("<td><a href='notice_list.jsp?page=" + (no_page_start-1) + "'>&lt;</a></td>"); // < 출력하고 이전 페이징 그룹의 마지막 페이지 링크
        }
        ...
        for (int i = no_page_start ; i < no_page_start + size_group_page ; i++) {	// 현재 페이지 번호가 속한 페이징의 그룹의 (첫 페이지 번호) 부터 (첫 페이지 번호+페이징 그룹의 크기) 까지 반복
          ...
          if (i == no_page_now) {					// 현재에 해당하는 페이지 번호이면 굵은 글씨로 표시
            out.println("<td><b><a href='notice_list.jsp?page=" + i + "'>" + i + "</a></b></td>");	// 현재 페이징 그룹에 해당하는 페이지 번호 출력
          } else {								// 아니면 일반 굵기로 표시
            out.println("<td><a href='notice_list.jsp?page=" + i + "'>" + i + "</a></td>");	// 현재 페이징 그룹에 해당하는 페이지 번호 출력
          }
        }
        ...
        if (!(no_page_start <= no_page_last && no_page_last <= no_page_start + size_group_page) && (no_page_last!=0)) {	// 마지막 페이징 그룹이 아닐 때와 출력할 데이터가 있을 때만 >와 >> 표시
          out.println("<td><a href='notice_list.jsp?page=" + (no_page_start + size_group_page) + "'>&gt;</a></td>"); // > 출력하고 다음 페이징 그룹의 첫 페이지 링크
          out.println("<td><a href='notice_list.jsp?page=" + no_page_last + "'>&gt;&gt;</a></td>");				// >> 출력하고 마지막 페이지로 이동 링크
        }
